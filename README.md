...
- **Orb system with crystal progression** — eight unlockable orbs (Drifter, Phantom, Inferno, Warp, Bulwark, Roamer, Phantasm, Helios), each with its own passive bonus and a unique burst ability. Runs earn 💎 Drift Crystals (score × difficulty multiplier, persisted in `localStorage`) that are spent in the shop to unlock new orbs. The three enhanced orbs — Roamer, Phantasm, and Helios — are instead earned through the **Sirius** progression: cross a per-base-orb kill threshold (Drifter 2 000 / Phantom 3 000 / Inferno 4 000) and Sirius's unlock arc fires
- **Per-orb skill trees** — every orb has a three-path tree (e.g. Drifter's Chance / Cadence / Convergence). **As of v1.9.0 all seven orbs use a single-rank + active-point-budget model** — the one Roamer and Phantasm pioneered: each node is unlocked once with crystals, then toggled active/inactive within a limited active-point budget, so build choice (not grind) defines a run. All five original orbs — **Drifter, Phantom, Inferno, Warp, Bulwark** — have now been migrated alongside Roamer and Phantasm (budgets — Roamer/Phantasm base 25 / cap 30; the five original orbs base 20 / cap 25, expandable with crystals). Respec returns 100% of invested crystals for a 20% fee
- **Burst mechanic** — press both sides at once to fire the equipped orb's signature burst ability on a 30-second cooldown (unified across all orbs in v1.6.4). Every orb's burst resolves differently (instant powerup grant, chain destruction, shield refresh, stacked hyperspeed, etc.) and several trees layer their own cooldown reductions on top (Drifter's Metronome and Steady Hands, Inferno's Coronal Hold, Warp's Momentum)
- **First-time tutorial (v1.6.4)** — a 13-step guided sequence on first launch teaches attract / repel, the four base powerups, pair combos, the burst gesture, the streak meter, and the wall-death warning. Persistent flag in `localStorage` (synced via PGS) ensures it fires once per pilot
- **Sirius death-narrator + revive system (v2.0.0)** — a three-beat narrative arc on death: a fully-powered **Ascension** run on your first death, a one-time second-death fake-out, then **Sirius's** reveal and a guaranteed revive. Later deaths roll a difficulty-scaled revive chance — most a one-line revive, ~30% a multiple-choice cosmic quiz where a right *or* charmingly-wrong answer saves you (a dull-wrong answer ends the run). Survival-tiered death-screen roasts fire when no revive procs
- **Custom phase events** — beyond the rotating spawn pool, eligible phases can roll a scripted, pre-warned event that locks spawns to a single themed body for its duration: METEOR SHOWER (phase 2), WAILING CHORUS or THE WRITHING DEEP (phase 3), and HALL OF MIRRORS (phase 4), with per-difficulty roll chances
...
- **Full additive visual overhaul (v2.0.0)** — every effect family (novas, the EMP family, the hyperspeed barrier, Phantom Blast, planet ripples, player auras, pickup/destroy bursts, crack fragments, pair tethers, floating proc/combo labels) was rebuilt on a single additive recipe: constant-thickness vector strokes plus baked soft-sprite blooms, with no `shadowBlur`, per-frame gradients, or full-screen fills — crisper at every resolution and far cheaper on long sessions
...
- **Streak + time-bonus scoring** — destroy a planet via Nova, Hyperspeed barrier, or Phantom Blast and your next destroy is worth more (streak × 4 flat, cap 8 — Drifter's Prime Mover raises this to 12); survive longer and a time bonus adds a score-proportional payout every 30 seconds (15s with Drifter's Tempo)
...
- **4 difficulties** — Normal, Hard, Extreme, and **Impossible** (unlocked by beating Extreme's apocalypse). Speed cap is reached at 2:00 across the first three (was 3:00 pre-v1.6.4 — the early game is snappier without changing the cap value); Impossible hits cap at 1:00 and continues ramping +10%/min and +1 max body every 2 minutes post-cap, with a stricter backend score allowlist
