...
- **Orb system with crystal progression** — seven unlockable orbs (Drifter, Phantom, Inferno, Warp, Bulwark, Roamer, Phantasm), each with its own passive bonus and a unique burst ability. Runs earn 💎 Drift Crystals (score × difficulty multiplier, persisted in `localStorage`) that are spent in the shop to unlock new orbs
- **Per-orb skill trees** — every orb has a three-path tree (e.g. Drifter's Chance / Cadence / Convergence). **As of v1.9.0 the orbs are being reworked, one at a time, to a single-rank + active-point-budget model** — the one Roamer and Phantasm already use: each node is unlocked once with crystals, then toggled active/inactive within a limited active-point budget, so build choice (not grind) defines a run. Reworked so far: **Roamer, Phantasm, Drifter, Phantom, Inferno** (budgets — Roamer/Phantasm base 25 / cap 30; the reworked originals Drifter, Phantom & Inferno base 20 / cap 25, expandable with crystals). Warp and Bulwark are still on the older multi-rank trees pending their rework. Respec returns 100% of invested crystals for a 20% fee
